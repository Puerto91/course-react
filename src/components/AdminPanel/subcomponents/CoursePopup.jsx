...
const CoursePopup = ({
  ...
  const [formAuthors, setFromAuthors] = useState(authors);
  ...
  const authorsElements = formAuthors.map((author) => (
    <li key={author} className={style("list-item")}>
      <p className={style("list-item-author")}>{author}</p>
      ...
      <div className={style()}>
        ...
        >
          ...
          <div className={style("options-menu")}>
            <button className={style("options-button")} type="submit">
              {correctLabel}
            </button>
            <button className={style("options-button")} onClick={hidePopup}>
              Anuluj
            </button>
          </div>
        </form>
        <p className={style("form-row")}>Lista autorów:</p>
        <ul className={style("list")}>{authorsElements}</ul>
      </div>
    </Modal>
  );
};
